...
function BenefitCard({ benefit, index }: { benefit: (typeof benefits)[number]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`reveal reveal-delay-${(index % 4) + 1} ${isVisible ? 'is-visible' : ''} flex items-start gap-4 p-5 rounded-2xl hover:bg-white/[0.03] transition-colors duration-300 group`}
    >
      <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-navy-600/20 to-navy-800/20 border border-navy-500/20 flex items-center justify-center group-hover:from-accent-500 group-hover:to-accent-600 group-hover:border-accent-400/30 transition-all duration-500 group-hover:scale-110">
        <benefit.icon className="w-6 h-6 text-navy-300 group-hover:text-white transition-colors duration-500" />
      </div>
      <div>
        ...
        <p className="text-sm text-ink-300 leading-relaxed">{benefit.desc}</p>
      </div>
    </div>
  );
}
